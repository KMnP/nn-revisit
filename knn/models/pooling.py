#!/usr/bin/env python3
"""
credit: https://github.com/filipradenovic/cnnimageretrieval-pytorch/tree/master/cirtorch/layers  # noqa
"""
import math
...
def spoc(x):
    return F.avg_pool2d(x, (x.size(-2), x.size(-1))).squeeze(-1).squeeze(-1)
    # return F.adaptive_avg_pool2d(x, (1,1)) # alternative


def gem(x, p=3, eps=1e-6):
    return F.avg_pool2d(
        x.clamp(min=eps).pow(p), (x.size(-2), x.size(-1))).pow(1. / p)
    # alternative
...
def roipool(x, rpool, L=3, eps=1e-6):
    ...
    for l in range(1, L + 1):
        ...
        if l + Wd == 1:
            ...
            b = (W - wl) / (l + Wd - 1)
        cenW = torch.floor(wl2 + torch.Tensor(
            range(l - 1 + Wd + 1)) * b).int() - wl2  # center coordinates
        if l + Hd == 1:
            b = 0
        else:
            b = (H - wl) / (l + Hd - 1)
        cenH = torch.floor(
            wl2 + torch.Tensor(range(l - 1 + Hd + 1)) * b
        ).int() - wl2  # center coordinates
